/**
 * sql-query-worker.js - Web Worker for Qwen model inference
 * Handles model loading and text generation for SQL query generation
 */
...
async function load(device = "webgpu") {
  ...
  try {
    const [tokenizer, model] = await SQLPipeline.getInstance((x) => {
    ...
    if (errorStr.includes("[WebGPU]") && errorStr.includes("Failed to generate kernel")) {
      ...
      userMessage = "WebGPU ran out of memory.";
    }
 ...
 * @param {Object} data - { nodeId, prompt, maxNewTokens }
 */
async function generate({ nodeId, prompt, maxNewTokens = 2048 }) {
  try {
    const [tokenizer, model] = await SQLPipeline.getInstance();
    ...
    if (errorStr.includes("[WebGPU]") && errorStr.includes("Failed to generate kernel")) {
      errorType = 'context-limit';
      userMessage = "The input is too long for WebGPU to process.";
    } else if (errorStr.includes("out of memory") || errorStr.includes("OOM")) {
      errorType = 'context-limit';
      userMessage = "WebGPU ran out of memory.";
    }
